...
ll solve()
{
    ...
    while (size_j != 0)
    {
        ...
        int bag_size = sizeof(bag)/12;
        multiset<ll>::iterator it = bag.lower_bound(weight);
        if (it == bag_size) continue;
        else
        {
            res += value;
            bag.erase(it);
...
void init()
{
    ...
    for (int i=0; i<n; i++)
    {
        ...
        cin >> weight >> value;
        jwrl[i].first = value;
        jwrl[i].second = weight;
    }
    
    sort(jwrl.begin(), jwrl.end(), cmp);
...
ll solve()
{
    ...
    for (int i=0; i<n; i++)
    {
        ll value = jwrl[i].first;
        ll weight = jwrl[i].second;
