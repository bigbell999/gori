...
int find_win( int a )
{
    ...
    if (a == build_2_win || a == pt[a] ) return a;
    return pt[a] = find_win(pt[a]);
}
...
int main()
{
    ...
    for (int k=0; k<t; k++)
    {
        ...
        for (int i=1; i<=n; i++) 
        {
            if ( (tech[i] < tech_2_win) && (pt[i] == build_2_win) ) tech_time[tech[i]].push_back( demand[i] );
        }
